...
app.controller('AppCtrl', function($scope, $ionicModal, $timeout, Services, Constant, UiServices, $http, Additional_services, $filter, $localStorage, $rootScope, $state, $cordovaDatePicker, $ionicHistory, $ionicPlatform, $ionicPopup) {
  ...
  Services.webServiceCallPost(sending_data, 'get_pass_changed_status').then(function(response)
  {
	...
	$scope.product_catalogue=function()
	{
		//product category page open 

      $state.go('app.product_catalogue', '');
...
app.controller('product_catelogueCtrl', function($scope, $rootScope, $ionicHistory, $ionicModal, Services, $state, UiServices, $localStorage, Constant){

  $scope.base_url=Constant.base_url.category_url;


	UiServices.show_loader();
	Services.webServiceCallPost('', 'get_categories').then(function(response)
  {
      if(response.data[1].response.status==1)
      {

        $scope.product_category=response.data[0];
    		  UiServices.hide_loader();
      }
  });
  ...
  $scope.category_name_clicked=function(category_id)
  {


    $ionicModal.fromTemplateUrl('templates/product_details_category.html', 
    {
      scope: $scope
    }).then(function(modal) 
    {
      $scope.category_details_modal = modal;
    });
    ...
    UiServices.show_loader();
    Services.webServiceCallPost(cate, 'get_products_from_category_id').then(function(response)
    {
        UiServices.hide_loader();
        if(response.data[1].response.status==1)
        {
          $scope.image_url=Constant.base_url.image_url;
          $scope.product_details=response.data[0].data;
          //console.log(JSON.stringify($scope.product_details));
          $scope.category_details_modal.show();
    ...
    $scope.close_product_cat_details_model=function()
    {
      $scope.category_details_modal.hide();
    }
